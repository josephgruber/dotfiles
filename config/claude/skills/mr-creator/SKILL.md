...
name: mr-creator
description:
  Use this skill when asked to create a merge request (MR). It ensures all MRs
  follow the repository's established templates and standards.
---

# Merge Request Creator

This skill guides the creation of high-quality Merge Requests that adhere to the
repository's standards.

## Workflow

Follow these steps to create a Merge Request:
...
3.  **Locate Template**: Search for a merge request template in the repository.
    - Check `.gitlab/merge_request_templates/DEFAULT.md`
    - If multiple templates exist (e.g., in `.gitlab/merge_request_template/MERGE_REQUEST_TEMPLATE/`),
...
5.  **Draft Description**: Create a MR description that strictly follows the
    ...
    - **Content**: Fill in the sections with clear, concise summaries of your
      changes.
    - **Related Issues**: Link any issues fixed or related to this MR (e.g.,
      "Fixes #123").

6.  **Preflight Check**: Before creating the MR, run the workspace pre-commit
...
8.  **Create MR**: Use the `glab` CLI to create the MR. To avoid shell escaping
    ...
    # 2. Create the MR using the --description flag
    glab mr create --title "type(scope): succinct description" --description "$(cat <temp_file_path>)"
